...
{
    /**
     * عرض قائمة حجوزات خدمات المنزل الخاصة بالبائع
     */
    public function index(Request $request)
    {
        ...
        $query = HomeServiceBooking::whereHas('homeService', function ($q) use ($seller) {
            $q->where('seller_id', $seller->id);
        })->with(['homeService', 'customer', 'employee']);
    ...
    public function show($id)
    {
        ...
        $booking = HomeServiceBooking::with(['homeService', 'customer', 'employee', 'payment'])
            ->whereHas('homeService', function ($q) use ($seller) {
                $q->where('seller_id', $seller->id);
            })
            ->find($id);
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $booking = HomeServiceBooking::whereHas('homeService', function ($q) use ($seller) {
            $q->where('seller_id', $seller->id);
        })->find($id);
    ...
    public function store(Request $request)
    {
        ...
        $homeServiceExists = $seller->homeServices()
            ->where('id', $request->home_service_id)
            ->exists();

        if (!$homeServiceExists) {
            return $this->sendError('الخدمة غير موجودة أو لا تنتمي لك', [], 404);
        }

        $booking = HomeServiceBooking::create($request->all());
