...
class StudioServiceBookingController extends BaseController
{
    ...
    /**
     * عرض قائمة حجوزات خدمات الاستوديو الخاصة بالعميل
     */
    public function index(Request $request)
    {
        ...
        $query = StudioServiceBooking::where('customer_id', $customer->id)
            ->with(['studioService', 'studioService.seller']);
    ...
    public function store(Request $request)
    {
        ...
        $studioService = StudioService::find($request->studio_service_id);
        if (!$studioService || $studioService->booking_status !== 'available') {
            return $this->sendError('الخدمة غير متوفرة للحجز حالياً', [], 400);
        }

        // إنشاء الحجز
        $booking = new StudioServiceBooking();
        $booking->customer_id = $customer->id;
        $booking->studio_service_id = $request->studio_service_id;
        $booking->seller_id = $studioService->seller_id; // إضافة معرف البائع من الخدمة
        ...
        $booking->paid_amount = $studioService->discount ? $studioService->discounted_price : $studioService->price; // تعيين قيمة الدفع من سعر الخدمة
        ...
        return $this->sendResponse($booking->load(['studioService', 'studioService.seller']), 'تم إنشاء الحجز بنجاح');
    }
    ...
    public function show($id)
    {
        ...
        $booking = StudioServiceBooking::with(['studioService', 'studioService.seller', 'employee', 'payment'])
    ...
    public function update(Request $request, $id)
    {
        ...
        $booking = StudioServiceBooking::where('customer_id', $customer->id)->find($id);
        if (!$booking) {
            return $this->sendError('الحجز غير موجود', [], 404);
        }
        ...
        return $this->sendResponse($booking->load(['studioService', 'studioService.seller']), 'تم تحديث الحجز بنجاح');
    }
