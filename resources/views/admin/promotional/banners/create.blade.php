        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header" style="background-color: #2F3E3B; color: white;">
                    <h3 class="card-title">إضافة بانر إعلاني جديد</h3>
                    ...
                    <form action="{{ route('admin.promotional.banners.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="target_audience" name="target_audience" required>
                                    ...
                                    <option value="sellers" {{ old('target_audience') == 'sellers' ? 'selected' : '' }}>مقدمي الخدمات فقط</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="action_text" name="action_text" value="{{ old('action_text') }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="link_type" name="link_type" required>
                                    <option value="url" {{ old('link_type') == 'url' ? 'selected' : '' }}>رابط خارجي</option>
                                    <option value="seller" {{ old('link_type') == 'seller' ? 'selected' : '' }}>مقدم خدمة</option>
                                    <option value="home_service" {{ old('link_type') == 'home_service' ? 'selected' : '' }}>خدمة منزلية</option>
                                    <option value="studio_service" {{ old('link_type') == 'studio_service' ? 'selected' : '' }}>خدمة في الاستوديو</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3 link-fields" id="url-fields">
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" id="action_url" name="action_url" value="{{ old('action_url') }}">
                            </div>
                        </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select" id="linked_seller_id" name="linked_seller_id">
                                    ...
                                    @foreach(\App\Models\Seller::where('status', 'active')->orderBy('first_name')->get() as $seller)
                                        <option value="{{ $seller->id }}" {{ old('linked_seller_id') == $seller->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select" id="linked_home_service_id" name="linked_home_service_id">
                                    ...
                                    @foreach(\App\Models\HomeService::with('seller')->orderBy('name')->get() as $service)
                                        <option value="{{ $service->id }}" {{ old('linked_home_service_id') == $service->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select" id="linked_studio_service_id" name="linked_studio_service_id">
                                    ...
                                    @foreach(\App\Models\StudioService::with('seller')->orderBy('name')->get() as $service)
                                        <option value="{{ $service->id }}" {{ old('linked_studio_service_id') == $service->id ? 'selected' : '' }}>
                                            {{ $service->name }} - {{ $service->seller->name ?? 'غير معروف' }} ({{ $service->price }} ريال)
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <div class="form-check form-switch">
                                    ...
                                    <input class="form-check-input" type="checkbox" id="is_limited_time" name="is_limited_time" value="1" {{ old('is_limited_time') == '1' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="is_limited_time">عرض لفترة محدودة</label>
                                </div>
                            </div>
                        </div>
                        
                        <div class="row mb-3 limited-time-fields" style="display: none;">
                            <div class="col-md-6">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showLinkFields(linkType) {
            const refreshSelect2 = (selector) => {
                // تأكد من ضبط العرض بشكل صحيح عند إظهار العنصر
